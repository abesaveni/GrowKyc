...
async def get_communications_analytics(
    period: str = "30d", current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        logger.info(
            f"User {current_user.email} requested communications analytics "
            f"for period: {period}"
        )
        ...
        return {"analyticsData": analytics_data, "summary": summary}
