...
class DocumentExtraction(Base):
    ...
    mrz_data = Column(
        JSON, nullable=True, comment="Parsed MRZ components if applicable"
    )
...
class DocumentVerification(Base):
    ...
    tenant_id = Column(
        Integer, ForeignKey("tenants.id", ondelete="NO ACTION"), nullable=False
    )
    document_id = Column(
    ...
    verification_outcome = Column(
        String(50), nullable=True, comment="pass|fail|caution"
    )
...
class DocumentFraudCheck(Base):
    ...
    fraud_indicators = Column(
        JSON, nullable=True, comment="Array of specific fraud flags raised"
    )
...
class DocumentLiveness(Base):
    ...
    liveness_outcome = Column(
        String(50), nullable=True, comment="pass|fail|spoof_detected"
    )
    normalized_confidence = Column(Float, nullable=True)

    match_score_to_id = Column(
        Float, nullable=True, comment="If matched against ID portrait"
    )
