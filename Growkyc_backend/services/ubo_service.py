...
from models import BeneficialOwner, EntityDirector, OwnershipRelationship
from schemas import (
    BeneficialOwnerCreate,
    EntityDirectorCreate,
    OwnershipRelationshipCreate,
)
...
class UBOService:
    ...
    def add_beneficial_owner(
        self, client_id: int, ubo_data: BeneficialOwnerCreate
    ) -> BeneficialOwner:
        ...
        try:
            ...
            # Real deployments should trigger this through the Celery task.
            # Direct import is skipped to avoid circular dependencies.
            return ubo
    ...
    def add_entity_director(
        self, client_id: int, director_data: EntityDirectorCreate
    ) -> EntityDirector:
    ...
    def add_ownership_relationship(
        self, relationship_data: OwnershipRelationshipCreate
        ...
        if self._is_circular_ownership(
            relationship_data.child_owner_id, relationship_data.parent_owner_id
        ):
            raise ValidationError(
                "Adding this relationship creates a circular ownership structure."
            )

        try:
            rel = OwnershipRelationship(
                tenant_id=tenant_id, **relationship_data.model_dump(exclude_unset=True)
            )
            self.db.add(rel)
            self.db.commit()
            self.db.refresh(rel)
    ...
    def _is_circular_ownership(
        self, start_id: int, target_id: int, visited: set = None
    ) -> bool:
        """
        ...
        children_rels = (
            self.db.query(OwnershipRelationship)
            .filter(OwnershipRelationship.parent_owner_id == start_id)
            .all()
        )
    ...
    def get_ownership_tree(self, client_id: int) -> Dict[str, Any]:
        ...
        ubos = (
            ...
            .filter(BeneficialOwner.client_id == client_id)
            .all()
        )
        ...
        return {
            "client_id": client_id,
            "nodes": [
                {"id": u.id, "name": u.full_name, "type": u.ownership_type}
                for u in ubos
            ],
            "edges": [
                {
    ...
    def aggregate_ubo_summary(self, client_id: int) -> Dict[str, Any]:
        ...
        total_percentage = sum(
            [
                ...
                for u in ubos
                if u.ownership_percentage is not None and u.ownership_type == "direct"
            ]
        )
