...
from models import Client, IndividualProfile, EntityProfile, User
from schemas import IndividualProfileCreate, EntityProfileCreate
...
class ClientService:
    ...
    def create_individual_client(
        self, user_id: int, profile_data: IndividualProfileCreate, trigger_async: bool = False
    ) -> Client:
        """
        ...
        try:
            ...
            name = " ".join(filter(None, [profile_data.first_name, profile_data.middle_name, profile_data.last_name]))
            if not name:
    ...
    def create_entity_client(
        self, user_id: int, profile_data: EntityProfileCreate, trigger_async: bool = False
    ) -> Client:
        """
    ...
    def _trigger_onboarding_engines(self, client: Client, trigger_async: bool) -> None:
        ...
        if trigger_async:
            ...
            ScreeningService(self.db).perform_screening(client, triggered_by_user_id=client.user_id)
            RiskService(self.db).calculate_risk(client, user_id=client.user_id, trigger="onboarding")
