// ── Pre-configured email notification service (SES) ───────────────────────────
// Import here so server-side consumers can do:
//   import { emailNotificationService } from '../../lib/notifications/index';
import { EmailNotificationAdapter } from './adapters/email/emailNotificationAdapter';
import { SesEmailExecutionPort } from './adapters/email/sesEmailExecutionPort';

export const emailNotificationService = new EmailNotificationAdapter({
  execution_port: new SesEmailExecutionPort(),
});

export { SesEmailExecutionPort } from './adapters/email/sesEmailExecutionPort';

// ── Model / service re-exports ────────────────────────────────────────────────
