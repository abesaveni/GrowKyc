...
export function FraudDetectionSettings() {
  ...
  const [fraudRules, setFraudRules] = useState<FraudRule[]>([
    ...
    },
    {
      ...
      actions: ['Create Case', 'Freeze Account', 'Report to AUSTRAC', 'Investigate Network']
    }
  ]);
  ...
  const toggleRule = (ruleId: string) => {
    setFraudRules(fraudRules.map(rule => 
  ...
  const toggleCaseTrigger = (triggerId: string) => {
    setCaseTriggers(caseTriggers.map(trigger => 
  ...
  const toggleNotification = (notificationId: string) => {
    setNotificationSettings(notificationSettings.map(notification => 
