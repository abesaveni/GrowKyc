...
The adapter-based backend uses environment variables for all provider secrets.
...
Local setup: run `npm install`, copy `.env.example` to `.env`, populate provider credentials, then run `npm run dev`. Production setup should set `APP_ENV=production` so startup readiness checks validate critical AWS, OAuth, API, and webhook configuration before the app starts serving traffic.
