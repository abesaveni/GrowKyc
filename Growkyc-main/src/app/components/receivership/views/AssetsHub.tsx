...
export function AssetsHub({ matterId }: { matterId?: string }) {
  ...
  return (
    <div className="min-h-full">
      <div className="border-b border-white/10 bg-white px-6 pt-4">
        <div className="flex gap-1">
          <button
            type="button"
            onClick={() => setTab('realisation')}
            className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 ${
              tab === 'realisation' ? 'border-red-600 text-red-300' : 'border-transparent text-slate-300'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 ${
              tab === 'register' ? 'border-red-600 text-red-300' : 'border-transparent text-slate-300'
            }`}
          >
