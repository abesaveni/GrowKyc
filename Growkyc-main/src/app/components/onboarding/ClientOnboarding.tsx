...
export function ClientOnboarding({ onComplete }: { onComplete?: () => void }) {
            ...
            <div key={phase.id} className="flex items-center flex-1">
              <div className="flex flex-col items-center flex-1">
                ...
                <p className={`text-xs font-semibold text-center ${
                  isActive ? 'text-blue-300' : isPast ? 'text-green-300' : 'text-slate-300'
                }`}>
                  {phase.label}
                </p>
              </div>
  ...
  const renderPhase1Structure = () => (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-100">Phase 1: Build Your Structure</h2>
          <p className="text-slate-300">Add all entities that will be using our services</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {entities.length === 0 ? (
          <div className="col-span-full text-center py-16 bg-white border-2 border-dashed border-white/10 rounded-lg">
            <Building2 className="w-16 h-16 text-gray-400 mx-auto mb-4" />
            <h3 className="text-xl font-bold text-slate-100 mb-2">No Entities Added Yet</h3>
            <p className="text-slate-300 mb-6">Click "Add Entity" to start building your structure</p>
          ...
          entities.map((entity) => {
            ...
            return (
              <div key={entity.id} className={`bg-white border-2 rounded-lg p-6 transition-colors ${
                isNameEmpty ? 'border-amber-400 shadow-amber-100 shadow-lg' : 'border-white/10 hover:border-blue-300'
              }`}>
                {isNameEmpty && (
                  <div className="mb-3 flex items-center gap-2 text-amber-400 text-sm font-semibold">
                  ...
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <div className="flex-1">
                      <input
                        ...
                        className={`w-full font-bold text-slate-100 border-b focus:border-blue-600 outline-none ${
                          isNameEmpty ? 'border-amber-400 placeholder-amber-500' : 'border-white/10'
                        }`}
                      />
                      <p className="text-xs text-slate-300 mt-1">{entityType?.label}</p>
                    </div>
                  </div>
                  <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-6">
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-slate-100">Select Entity Type</h2>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-green-500/10 border-2 border-green-500/30 rounded-lg p-6">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="font-bold text-green-300 text-lg">All Onboarding Steps Complete</h3>
          </div>
          ...
          <div className="bg-white rounded-lg p-4 border border-green-300">
            <p className="text-sm text-slate-300 mb-2"><strong>Onboarding Summary:</strong></p>
        ...
        <div className="bg-white border-2 border-white/10 rounded-lg p-6">
          <h3 className="font-bold text-slate-100 mb-4">Entity Summary</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {entities.map((entity) => {
              const entityType = entityTypes.find(t => t.id === entity.type);
              const Icon = entityType?.icon || Building2;
              ...
              return (
                <div key={entity.id} className="border border-white/10 rounded-lg p-4">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-300">{entityType?.label}</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-slate-300">Services:</span>
                      <span className="font-semibold text-slate-100">{entity.services.length}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-slate-300">Annual Value:</span>
                      <span className="font-semibold text-slate-100">${entity.pricing.toLocaleString()}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
        ...
        {/* Declaration */}
        <div className="bg-white border-2 border-white/10 rounded-lg p-6">
          <h3 className="font-bold text-slate-100 mb-4">Declaration</h3>
          
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4 mb-4">
            <p className="text-sm text-amber-300 font-semibold mb-2">Please Read Carefully:</p>
          ...
          <label className="flex items-start gap-3 p-4 border-2 border-white/10 rounded-lg cursor-pointer hover:border-blue-400 transition-colors">
            <input
              type="checkbox"
              checked={declaration}
  ...
  return (
    <div className="min-h-screen bg-white/5 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-100 mb-2">Client Onboarding</h1>
          <p className="text-slate-300">AUSTRAC-compliant AML/CTF onboarding process</p>
        </div>
