...
export function EngagementLetter({ entities, onBack, onContinue }: any) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Phase 3: Engagement & Payment</h2>
        <p className="text-gray-600">Review and sign your engagement letter</p>
      </div>

      <div className="bg-white border-2 border-gray-200 rounded-lg p-8 max-h-[600px] overflow-y-auto">
        <h3 className="text-xl font-bold text-gray-900 mb-6">Engagement Letter</h3>
        ...
        <div className="prose max-w-none space-y-4 text-sm">
          ...
          <div className="space-y-4 my-6">
            {entities.map((entity: any) => (
              <div key={entity.id} className="p-4 bg-gray-50 rounded-lg border-2 border-gray-200">
                <h4 className="font-bold text-gray-900">{entity.name}</h4>
                <p className="text-sm text-gray-600 mt-1">Entity Type: {entity.type}</p>
                <div className="mt-2">
          ...
          <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-4 my-6">
            <h4 className="font-bold text-blue-900 mb-2">Total Investment</h4>
            <div className="flex justify-between text-blue-900">
              <span>Annual Total:</span>
              <span className="text-xl font-bold">${totalAnnual.toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-blue-700 text-sm mt-1">
          ...
          <div className="space-y-3 text-gray-700">
            <h4 className="font-bold text-gray-900 text-base">Terms & Conditions:</h4>
          ...
          <div className="bg-amber-50 border-2 border-amber-200 rounded-lg p-4 my-6">
        ...
        <div className="border-t-2 border-gray-300 pt-6 mt-6">
          ...
          <div className="mb-6">
            ...
            <div className="border-2 border-gray-300 rounded-lg h-32 flex items-center justify-center bg-gray-50">
...
export function InternalReviewAndApproval({ entities, onApprove, onReject, onBack, onContinue, canProgress }: any) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Phase 5: Internal Review & Manager Approval</h2>
        <p className="text-gray-600">Review compliance checks and approve or reject each entity</p>
      </div>
        ...
        <table className="w-full">
          <thead className="bg-gray-50 border-b-2 border-gray-200">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase">ID Verified</th>
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase">UBO</th>
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase">Sanctions</th>
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase">PEP</th>
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase">Risk</th>
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase">Approval</th>
          ...
          <div className="grid grid-cols-2 gap-6 mb-6">
            <div>
              ...
              <div className="space-y-1 text-sm">
                <p><span className="text-gray-600">Type:</span> {selectedEntity.type}</p>
                <p><span className="text-gray-600">Risk Level:</span> {selectedEntity.riskStatus.toUpperCase()}</p>
                <p><span className="text-gray-600">Services:</span> {selectedEntity.services.length}</p>
                <p><span className="text-gray-600">Annual Fee:</span> ${selectedEntity.pricing.toLocaleString()}</p>
              </div>
            </div>
            <div>
              <h4 className="font-semibold text-gray-900 mb-2">Compliance Status</h4>
              <div className="space-y-1 text-sm">
                <p><span className="text-gray-600">ID Verified:</span> {selectedEntity.data.idDocuments ? '✓ Yes' : '✗ No'}</p>
                <p><span className="text-gray-600">UBO Complete:</span> {selectedEntity.data.beneficialOwners?.length > 0 || selectedEntity.data.fullLegalName ? '✓ Yes' : '✗ No'}</p>
                <p><span className="text-gray-600">Sanctions:</span> {selectedEntity.data.sanctionsResult || 'Pending'}</p>
                <p><span className="text-gray-600">PEP:</span> {selectedEntity.data.pepResult || 'Pending'}</p>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="space-y-6">
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 mb-8">All compliance checks passed - Ready for activation</p>

        <div className="max-w-3xl mx-auto space-y-4 mb-8">
          {entities.map((entity: any) => (
            <div key={entity.id} className="flex items-center justify-between p-4 bg-green-50 border-2 border-green-200 rounded-lg">
        ...
        <div className="max-w-2xl mx-auto bg-blue-50 border-2 border-blue-200 rounded-lg p-6 mb-8 text-left">
          <h3 className="font-bold text-blue-900 mb-4">Final Acknowledgement</h3>
