...
export function LeadScheduleEditor({ onBack }: LeadScheduleEditorProps) {
  ...
  const getRiskBadge = (risk: string) => {
    switch (risk) {
      ...
      case 'medium':
        return <span className="px-2 py-0.5 bg-amber-500/15 text-amber-300 text-xs font-semibold rounded">MED</span>;
      case 'low':
      ...
      <div className="bg-white border-b border-white/10 px-6 py-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-100">{scheduleData.name}</h1>
              <p className="text-xs text-slate-300">Lead Schedule • TB-Controlled • Auto-sync enabled</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-6 gap-4 text-sm">
          <div>
            ...
            <div className="font-semibold text-amber-300">IN REVIEW</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="font-semibold text-slate-100">{scheduleData.preparer}</div>
          </div>
          <div>
            <span className="text-slate-300 text-xs">Reviewer:</span>
            <div className="font-semibold text-slate-100">{scheduleData.reviewer}</div>
          </div>
          <div>
            <span className="text-slate-300 text-xs">Last Updated:</span>
            <div className="font-semibold text-slate-100">{scheduleData.last_updated}</div>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-green-500/10 border border-green-300 rounded px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-green-300">Trial Balance Linked</h3>
        ...
        <div className="grid grid-cols-4 gap-4">
          <div className="bg-white border border-white/10 rounded p-4">
            <div className="text-xs text-slate-300 mb-1">TB Balance</div>
            <div className="text-2xl font-bold text-slate-100 font-mono">${formatCurrency(scheduleData.tb_balance)}</div>
            <div className="text-xs text-green-400 font-semibold mt-1">Reconciled ✓</div>
          </div>
          <div className="bg-white border border-white/10 rounded p-4">
            <div className="text-xs text-slate-300 mb-1">Prior Year</div>
            <div className="text-2xl font-bold text-slate-300 font-mono">${formatCurrency(398200)}</div>
          ...
          <div className="bg-white border border-white/10 rounded p-4">
            <div className="text-xs text-slate-300 mb-1">Variance</div>
            <div className="text-2xl font-bold text-green-400 font-mono">${formatCurrency(scheduleData.variance_prior_year)}</div>
            <div className="text-xs text-slate-300 mt-1">Growth aligns with revenue</div>
          </div>
          <div className="bg-white border border-white/10 rounded p-4">
            <div className="text-xs text-slate-300 mb-1">High Risk Items</div>
            <div className="text-2xl font-bold text-red-400 font-mono">{highRiskCount}</div>
            <div className="text-xs text-red-400 font-semibold mt-1">Requires review</div>
          </div>
        </div>
        ...
        <div className="border border-white/10 rounded bg-white overflow-hidden">
          <div className="bg-white/5 border-b border-white/10 px-4 py-2">
            <h3 className="font-semibold text-slate-100">GL to Aged Analysis Reconciliation</h3>
          </div>
          <table className="w-full text-sm border-collapse">
            <tbody>
              <tr className="hover:bg-blue-500/10">
                ...
                <td className="border border-white/10 px-4 py-2 text-right text-slate-100 font-mono font-semibold">
              ...
              <tr className="hover:bg-blue-500/10">
                ...
                <td className="border border-white/10 px-4 py-2 text-right text-red-300 font-mono font-semibold">
              ...
              <tr>
                ...
                <td className="border border-white/10 px-4 py-2 text-right text-slate-300 font-mono">
                  ${formatCurrency(reconciliation.aged_analysis_total)}
                </td>
              </tr>
              <tr className="bg-green-500/10">
                <td className="border border-white/10 px-4 py-2 text-green-300 font-bold flex items-center gap-2">
                  <CheckCircle className="w-4 h-4" />
                  Variance
                </td>
                <td className="border border-white/10 px-4 py-2 text-right text-green-300 font-mono font-bold">
        ...
        <div className="border border-white/10 rounded bg-white overflow-hidden">
          <div className="bg-white/5 border-b border-white/10 px-4 py-2 flex items-center justify-between">
            <h3 className="font-semibold text-slate-100">Aged Receivables Analysis</h3>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-white/5">
                  <th className="border border-white/10 px-3 py-2 text-left font-semibold text-slate-300">Customer</th>
                  <th className="border border-white/10 px-3 py-2 text-left font-semibold text-slate-300">Invoice</th>
                  ...
                  <th className="border border-white/10 px-3 py-2 text-center font-semibold text-slate-300">Age (Days)</th>
                  <th className="border border-white/10 px-3 py-2 text-center font-semibold text-slate-300">Category</th>
                  <th className="border border-white/10 px-3 py-2 text-center font-semibold text-slate-300">Risk</th>
                  <th className="border border-white/10 px-3 py-2 text-center font-semibold text-slate-300">Evidence</th>
                </tr>
              </thead>
              <tbody>
                {receivablesDetail.map((item) => (
                  <tr key={item.id} className="hover:bg-blue-500/10">
                    ...
                    <td className="border border-white/10 px-3 py-2 text-right text-slate-100 font-mono font-semibold">
                      ${formatCurrency(item.amount)}
                    </td>
                    <td className="border border-white/10 px-3 py-2 text-center text-slate-100 font-mono">
                      {item.age_days}
                    </td>
                    <td className="border border-white/10 px-3 py-2 text-center text-slate-300 text-xs">
                      {item.category}
                    </td>
              ...
              <tfoot>
                <tr className="bg-white/5 font-bold">
                  <td colSpan={3} className="border border-white/10 px-3 py-2 text-right text-slate-100">
                    TOTAL:
                  </td>
                  <td className="border border-white/10 px-3 py-2 text-right text-slate-100 font-mono">
                    ${formatCurrency(totalReceivables)}
                  </td>
                  <td colSpan={4} className="border border-white/10"></td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        <div className="border border-white/10 rounded bg-white p-4">
          <h3 className="font-semibold text-slate-100 mb-3">Linked Trial Balance Accounts</h3>
          <div className="space-y-2">
            <div className="flex items-center justify-between p-2 bg-white/5 rounded">
              <div className="flex items-center gap-3">
                <span className="px-2 py-1 bg-blue-500/15 text-blue-300 font-mono text-xs font-semibold rounded">1200</span>
                <span className="text-sm text-slate-100">Trade Debtors</span>
              </div>
              <span className="text-sm font-mono text-slate-100 font-semibold">${formatCurrency(432800)}</span>
            </div>
            <div className="flex items-center justify-between p-2 bg-white/5 rounded">
              <div className="flex items-center gap-3">
                <span className="px-2 py-1 bg-blue-500/15 text-blue-300 font-mono text-xs font-semibold rounded">1210</span>
                <span className="text-sm text-slate-100">Provision for Doubtful Debts</span>
              </div>
              <span className="text-sm font-mono text-red-300 font-semibold">($12,500)</span>
            </div>
          </div>
        </div>
      </div>
