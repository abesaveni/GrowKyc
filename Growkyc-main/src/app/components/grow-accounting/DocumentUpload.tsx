...
export function DocumentUpload({ onNavigate }: DocumentUploadProps) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'processing':
        return <span className="px-2 py-1 bg-blue-50 text-blue-700 text-xs font-semibold rounded border border-blue-200">Processing</span>;
      case 'complete':
        return <span className="px-2 py-1 bg-green-50 text-green-700 text-xs font-semibold rounded border border-green-200">Complete</span>;
      case 'error':
        return <span className="px-2 py-1 bg-red-50 text-red-700 text-xs font-semibold rounded border border-red-200">Error</span>;
      case 'pending':
        return <span className="px-2 py-1 bg-gray-50 text-gray-700 text-xs font-semibold rounded border border-gray-200">Pending</span>;
      default:
        return null;
    }
  };
    ...
    <WorkpaperLayout currentPage="documents" onNavigate={onNavigate}>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-[32px] font-bold text-gray-900">Documents</h1>
            <p className="text-sm text-gray-600 mt-1">Upload, manage, and extract data from documents</p>
          </div>
          <Button 
          ...
          <Card className="shadow-[0_4px_12px_rgba(0,0,0,0.06)]">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-gray-600">AI Extracted</p>
                  <p className="text-2xl font-bold text-gray-900">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex items-center gap-2 border border-gray-300 rounded-lg p-1">
                <button
                  ...
                  className={`p-1.5 rounded ${viewMode === 'grid' ? 'bg-[#2855a6] text-white' : 'text-gray-600 hover:bg-gray-100'}`}
                >
                  <Grid className="w-4 h-4" />
                </button>
                <button
                  onClick={() => setViewMode('list')}
                  className={`p-1.5 rounded ${viewMode === 'list' ? 'bg-[#2855a6] text-white' : 'text-gray-600 hover:bg-gray-100'}`}
                >
                  ...
                  {doc.extractedData && doc.ocrStatus === 'complete' && (
                    <div className="mb-3 p-3 bg-purple-50 border border-purple-200 rounded-lg">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-xs font-semibold text-purple-900">AI Extracted Data</p>
                      </div>
          ...
          <Card className="shadow-[0_4px_12px_rgba(0,0,0,0.06)]">
            <CardContent className="p-0">
              <div className="divide-y divide-gray-200">
                {filteredDocuments.map((doc) => (
                  <div key={doc.id} className="p-4 hover:bg-gray-50 cursor-pointer transition-colors">
