...
interface CaseSearchResult {
  ...
  borrowerResidentialAddress?: string;
...
function extractSearchResults(payload: any): CaseSearchResult[] {
...
export function ComprehensiveSearchResults({ 
  ...
  const caseSearchQuery = useMemo(() => {
    return [
      formData.caseNumber,
      formData.borrowerFirstName,
      formData.borrowerLastName,
      formData.companyName,
      formData.trustName,
      formData.propertyAddress,
      formData.titleReference,
  ...
  useEffect(() => {
    ...
    api.get('/api/v1/cases/search', { params: { q: caseSearchQuery } })
      .then((response) => {
        if (!isMounted) return;
        setCaseSearchResults(extractSearchResults(response.data));
      })
      .catch((error) => {
        if (!isMounted) return;
        setCaseSearchResults([]);
        setSearchError(error.apiError?.message || 'Case search is unavailable. Existing form data is still shown.');
      })
      .finally(() => {
        if (isMounted) {
  ...
  const caseSearchResult = caseSearchResults[0] || {};

  const borrowerData = {
    name: caseSearchResult.borrowerName || `${caseSearchResult.borrowerFirstName || formData.borrowerFirstName || 'John'} ${caseSearchResult.borrowerLastName || formData.borrowerLastName || 'Smith'}`,
    dob: caseSearchResult.borrowerDOB || formData.borrowerDOB || '1985-03-15',
    email: caseSearchResult.borrowerEmail || formData.borrowerEmail || 'john.smith@email.com',
    phone: caseSearchResult.borrowerPhone || formData.borrowerPhone || '0412 345 678',
    address: caseSearchResult.borrowerResidentialAddress || formData.borrowerResidentialAddress || '456 Residential Street, Sydney NSW 2000'
  };

  const guarantorData = {
    name: caseSearchResult.guarantorName || formData.guarantorName || 'Sarah Johnson',
    dob: caseSearchResult.guarantorDOB || formData.guarantorDOB || '1982-07-22',
    email: caseSearchResult.guarantorEmail || formData.guarantorEmail || 'sarah.johnson@email.com',
    phone: caseSearchResult.guarantorPhone || formData.guarantorPhone || '0423 456 789',
    address: caseSearchResult.guarantorResidentialAddress || formData.guarantorResidentialAddress || '789 Guarantee Lane, Melbourne VIC 3000'
  };
