  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg border-2 border-yellow-500/30 p-6">
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <h3 className="text-2xl font-bold text-slate-100">{sampleAssessment.clientName}</h3>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-lg font-bold text-slate-100">Overall Risk Score</h4>
                <div className="flex items-center gap-4">
              ...
              <div className="flex justify-between mt-2 text-xs font-semibold text-slate-300">
                <span>0 - Low</span>
                <span>25 - Medium</span>
                <span>50 - High</span>
            ...
            <div className="space-y-3">
              <h4 className="text-lg font-bold text-slate-100 mb-3">Risk Factor Breakdown</h4>
              {sampleAssessment.factors.map((factor, index) => (
                <div key={index} className="p-4 bg-white/5 rounded-lg border border-white/10">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-1">
                        <h5 className="font-semibold text-slate-100">{categoryLabels[factor.category]}</h5>
                        <span className="px-2 py-1 bg-white/10 text-slate-300 text-xs font-bold rounded">
                          Weight: {factor.weight}%
                        </span>
                      </div>
                      <p className="text-sm text-slate-300 mb-1"><strong>{factor.factor}</strong> - {factor.description}</p>
                      <div className="flex flex-wrap gap-2 mt-2">
                        {factor.evidence.map((evidence, evidenceIndex) => (
                          <span key={evidenceIndex} className="px-2 py-1 bg-blue-500/15 text-blue-300 text-xs rounded">
                            {evidence}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="text-right ml-4">
                      <p className="text-2xl font-bold text-slate-100">{factor.score}</p>
                      <p className="text-xs text-slate-300">max {factor.maxScore}</p>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-lg border border-white/10 p-6">
            <h3 className="text-xl font-bold text-slate-100 mb-4">Portfolio Risk Distribution</h3>
        ...
        <div className="space-y-6">
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-6">
            <div className="flex items-start gap-3">
              <Calculator className="w-6 h-6 text-blue-400 mt-0.5" />
              <div>
                <h3 className="font-bold text-blue-300 mb-2">Risk Assessment Calculator</h3>
          ...
          {riskFactorDefinitions.map((category, categoryIndex) => (
            <div key={categoryIndex} className="bg-white rounded-lg border border-white/10 p-6">
              <h3 className="text-xl font-bold text-slate-100 mb-4">{category.name}</h3>
              <div className="space-y-2">
                {category.factors.map((factor, factorIndex) => (
                  <label key={factorIndex} className="flex items-center gap-4 p-4 rounded-lg border-2 border-white/10 hover:border-orange-300 hover:bg-orange-500/10 cursor-pointer transition-colors">
                    <input type="radio" name={category.category} className="w-5 h-5 text-orange-400" />
                    <div className="flex-1">
                      <p className="font-semibold text-slate-100">{factor.name}</p>
                      <p className="text-sm text-slate-300">{factor.description}</p>
                    </div>
                    <div className={`px-4 py-2 rounded-lg font-bold ${
                      factor.baseScore === 0 ? 'bg-green-500/15 text-green-300' :
                      factor.baseScore <= 15 ? 'bg-yellow-500/15 text-yellow-300' :
                      factor.baseScore <= 30 ? 'bg-orange-500/15 text-orange-300' :
          ...
          <div className="bg-white rounded-lg border-2 border-orange-500 p-6">
            <h3 className="text-2xl font-bold text-slate-100 mb-4">Calculated Risk Score</h3>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-6xl font-bold text-orange-400">0</p>
                <p className="text-slate-300 mt-2">Select factors to calculate</p>
              </div>
      ...
      {activeTab === 'methodology' && (
        <div className="space-y-6">
          <div className="bg-white rounded-lg border border-white/10 p-6">
            <h3 className="text-2xl font-bold text-slate-100 mb-4">Risk Assessment Methodology</h3>
            
            <div className="prose max-w-none">
              <h4 className="text-lg font-bold text-slate-100 mt-6 mb-3">1. Risk-Based Approach</h4>
              ...
              <h4 className="text-lg font-bold text-slate-100 mt-6 mb-3">2. Risk Scoring Model</h4>
              <p className="text-slate-300 mb-4">
              ...
              <h4 className="text-lg font-bold text-slate-100 mt-6 mb-3">3. Risk Tier Assignment</h4>
              <div className="grid grid-cols-4 gap-4 my-6">
                <div className="p-4 bg-green-500/10 border-2 border-green-500/30 rounded-lg">
                  <p className="font-bold text-green-300 mb-2">LOW RISK</p>
                  <p className="text-3xl font-bold text-green-400 mb-2">0-24</p>
                  <p className="text-sm text-green-300">Standard CDD, Annual review</p>
                </div>
                <div className="p-4 bg-yellow-500/10 border-2 border-yellow-500/30 rounded-lg">
                  <p className="font-bold text-yellow-300 mb-2">MEDIUM RISK</p>
                  <p className="text-3xl font-bold text-yellow-400 mb-2">25-49</p>
                  <p className="text-sm text-yellow-300">Standard CDD, Semi-annual review</p>
                </div>
                <div className="p-4 bg-orange-500/10 border-2 border-orange-500/30 rounded-lg">
                  <p className="font-bold text-orange-300 mb-2">HIGH RISK</p>
                  <p className="text-3xl font-bold text-orange-400 mb-2">50-74</p>
                  <p className="text-sm text-orange-300">Enhanced CDD, Quarterly review</p>
                </div>
                <div className="p-4 bg-red-500/10 border-2 border-red-500/30 rounded-lg">
                  <p className="font-bold text-red-300 mb-2">CRITICAL RISK</p>
                  <p className="text-3xl font-bold text-red-400 mb-2">75-100</p>
                  <p className="text-sm text-red-300">Enhanced CDD, Senior approval, Monthly review</p>
                </div>
              </div>

              <h4 className="text-lg font-bold text-slate-100 mt-6 mb-3">4. Approval Requirements</h4>
              <ul className="list-disc pl-6 space-y-2 text-slate-300 mb-4">
              ...
              <h4 className="text-lg font-bold text-slate-100 mt-6 mb-3">5. Review Frequency</h4>
              <p className="text-slate-300 mb-4">
              ...
              <h4 className="text-lg font-bold text-slate-100 mt-6 mb-3">6. Documentation</h4>
              <p className="text-slate-300 mb-4">
