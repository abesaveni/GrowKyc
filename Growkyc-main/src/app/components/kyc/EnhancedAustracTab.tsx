...
export function EnhancedAustracTab({ clientName, smrs, summary }: EnhancedAustracTabProps) {
  ...
  const SectionHeader = ({ id, title, icon: Icon, priority }: { id: string; title: string; icon: React.ComponentType<{ className?: string }>; priority?: boolean }) => {
    ...
    return (
      <button
        ...
        className={`w-full p-4 flex items-center justify-between border-b-2 transition-colors ${isExpanded ? 'bg-cyan-50' : 'bg-white hover:bg-gray-50'
          }`}
      >
  ...
  if (summary.totalSMRs === 0 && summary.totalTTRs === 0 && !smr) {
    return (
      <div className="space-y-6">
        ...
        <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2"><Shield className="w-6 h-6 text-indigo-600" /> Reporting Obligations</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { type: 'SMR', name: 'Suspicious Matter Report', threshold: 'Any Amount', status: 'Up to Date', color: 'bg-green-100 text-green-800 border-green-300' },
            { type: 'TTR', name: 'Threshold Transaction Report', threshold: '≥ $10,000 AUD', status: 'Monitoring', color: 'bg-blue-100 text-blue-800 border-blue-300' },
            { type: 'IFTI', name: 'Intl. Funds Transfer', threshold: 'Any Amount', status: 'Monitoring', color: 'bg-purple-100 text-purple-800 border-purple-300' }
          ].map(ob => (
            <Card key={ob.type} className="border shadow-sm">
              <CardContent className="p-4 flex flex-col justify-between h-full">
                <div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="font-black text-lg text-slate-800">{ob.type}</span>
                    <Badge variant="outline" className={`${ob.color} border font-bold`}>{ob.status}</Badge>
                  </div>
                  <p className="text-xs text-slate-500 font-medium mb-4">{ob.name}</p>
                </div>
                <div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2 mb-4"><Shield className="w-6 h-6 text-indigo-600" /> Reporting Obligations Status</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { type: 'SMR', name: 'Suspicious Matter Report', threshold: 'Any Amount', status: summary.activeConcerns ? 'Action Required' : 'Up to Date', date: summary.lastReportDate, color: summary.activeConcerns ? 'bg-red-100 text-red-800 border-red-400' : 'bg-green-100 text-green-800 border-green-300' },
            { type: 'TTR', name: 'Threshold Transaction Report', threshold: '≥ $10,000 AUD', status: summary.totalTTRs > 0 ? 'Active Filing' : 'Monitoring', date: summary.totalTTRs > 0 ? summary.lastReportDate : 'N/A', color: summary.totalTTRs > 0 ? 'bg-yellow-100 text-yellow-800 border-yellow-300' : 'bg-blue-100 text-blue-800 border-blue-300' },
            { type: 'IFTI', name: 'Intl. Funds Transfer Instruction', threshold: 'Any Amount', status: 'Monitoring', date: 'N/A', color: 'bg-purple-100 text-purple-800 border-purple-300' }
          ].map(ob => (
            <Card key={ob.type} className={`border-2 ${ob.color.replace('bg-', 'border-').split(' ')[2]} shadow-sm bg-white dark:bg-slate-900`}>
              <CardContent className="p-4 flex flex-col justify-between h-full">
                <div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="font-black text-lg text-slate-800 dark:text-slate-100">{ob.type}</span>
                ...
                <div className="flex justify-between items-end">
                  <div>
                    <p className="text-[10px] text-slate-400 uppercase tracking-widest">Reporting Threshold</p>
                    <p className="font-bold text-sm text-slate-700 dark:text-slate-200">{ob.threshold}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] text-slate-400 uppercase tracking-widest">Last Report</p>
                    <p className="font-bold text-sm text-slate-700 dark:text-slate-200">{ob.date}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
      ...
      <Card className="border-2 border-slate-200 shadow-md">
        <CardHeader className="bg-slate-50 dark:bg-slate-800/50 border-b pb-4">
          <CardTitle className="text-lg flex justify-between items-center">
            <span className="flex items-center gap-2"><TrendingUp className="w-5 h-5 text-blue-600" /> AUSTRAC Submission Tracking: {smr?.smrNumber || 'GENERAL_COMPLIANCE'}</span>
            <Badge className="bg-slate-200 text-slate-800 hover:bg-slate-300">{smr?.filingDate || 'Current'}</Badge>
          </CardTitle>
        </CardHeader>
          ...
          <div className="relative flex justify-between items-center w-full max-w-4xl mx-auto">
            ...
            {[
              ...
              <div key={i} className="flex flex-col items-center bg-white dark:bg-slate-900 px-2">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center border-4 mb-2 transition-colors ${step.active ? 'bg-blue-600 border-blue-200 text-white' : 'bg-slate-100 border-slate-200 text-slate-400'}`}>
                  <step.icon className="w-4 h-4" />
                </div>
                <p className={`text-xs font-bold text-center ${step.active ? 'text-slate-800 dark:text-slate-200' : 'text-slate-400'}`}>{step.label}</p>
      ...
      <Card className="border-2 border-indigo-200 shadow-md bg-gradient-to-br from-white to-indigo-50 dark:from-slate-900 dark:to-indigo-900/20">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Archive className="w-5 h-5 text-indigo-600" />
            Evidence Pack Builder
          </CardTitle>
          <p className="text-sm text-slate-600 dark:text-slate-400">Automatically assembles all required KYC documents, EDD reports, and transaction logs for AUSTRAC disclosure.</p>
        </CardHeader>
        <CardContent>
            ...
            <div className="space-y-4">
              ...
              {isAssembling || packReady ? (
                <div className="space-y-2">
                  <div className="flex justify-between text-xs font-bold text-indigo-800 dark:text-indigo-300">
                    <span>Assembly Progress</span>
                    <span>{Math.round(assemblyProgress)}%</span>
                  </div>
                  <Progress value={assemblyProgress} className="h-2 bg-indigo-200 [&>div]:bg-indigo-600" />
                </div>
              ) : null}
              ...
              <Button
                ...
                disabled={!packReady}
                variant="outline"
                className={`w-full ${packReady ? 'border-green-600 text-green-700 hover:bg-green-50' : 'opacity-50'}`}
              >
                <Download className="w-4 h-4 mr-2" /> Download Evidence PDF
              </Button>
            </div>

            <div className="bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-xl p-4 h-48 overflow-y-auto">
              ...
              <ul className="space-y-2">
                {checkedItems.map((item, idx) => (
                  <li key={idx} className="flex items-center justify-between text-sm animate-in fade-in slide-in-from-left-2">
                    ...
                    {item.status === 'warning' && (
                      <Badge variant="outline" className="text-[9px] bg-amber-50 text-amber-700 border-amber-200">Missing</Badge>
                    )}
                  </li>
                ))}
              </ul>
        ...
        <CardContent className="p-0">
          ...
          {!smr && (
            <div className="p-12 text-center bg-white border-b">
              ...
              <h4 className="text-lg font-bold text-slate-800 mb-2">Detailed Records Not Synced</h4>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {smr.suspiciousActivitySummary.primaryConcerns.map((concern, idx) => (
                        <div key={idx} className={`rounded-lg p-4 border-2 ${concern.severity === 'Critical' ? 'bg-red-50 border-red-300' :
