...
export function CaseControlCentre() {
  ...
  const kpis = {
    ...
    recentlyClosed: cases.filter(c => c.status === 'closed').length + 12
  };
      ...
      <CaseCreationModal
        ...
        onSuccess={(newCase) => {
          setCases(prev => {
            ...
            const manualOnly = updated.filter(c => c.id.startsWith('CASE-2026-') && c.triggerSource === 'Manual Referral');
            try {
              localStorage.setItem('cases_custom_cases', JSON.stringify(manualOnly));
            } catch (e) {
