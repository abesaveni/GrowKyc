...
export function GlobalSearch({ isOpen, onClose, onNavigate }: GlobalSearchProps) {
  ...
  useEffect(() => {
    ...
    if (q) {
      ...
      // Smart Natural Language Parsing / AI Matchers
      const wantsHighRisk = q.includes('high risk') || q.includes('high-risk') || q.includes('red flag');
      const wantsPep = q.includes('pep') || q.includes('politically');
      const wantsCdd = q.includes('cdd') || q.includes('due') || q.includes('expir');
      const wantsDocs = q.includes('document') || q.includes('passport') || q.includes('pdf');
      const wantsPolicy = q.includes('policy') || q.includes('governance') || q.includes('aml');

      if (wantsHighRisk) {
        filtered = filtered.filter(r => r.risk === 'high');
      } else if (wantsPep) {
        filtered = filtered.filter(r => (r.subtitle && r.subtitle.toLowerCase().includes('pep')) || r.title.toLowerCase().includes('smith'));
      } else if (wantsCdd) {
        filtered = filtered.filter(r => r.metadata.toLowerCase().includes('due') || r.metadata.toLowerCase().includes('verify'));
      } else if (wantsDocs) {
        filtered = filtered.filter(r => r.type === 'document' || r.type === 'client');
      } else if (wantsPolicy) {
        filtered = filtered.filter(r => r.type === 'policy');
      } else {
        // Fallback to broad multivariable textual search
        filtered = mockSearchResults.filter(r => 
  ...
  const handleCommandClick = (action: string) => {
    ...
    if (action === 'create_case') {
      onNavigate('case_management');
    } else if (action === 'verification') {
      onNavigate('kyc_dashboard_overview');
    } else if (action === 'navigate_clients') {
      onNavigate('client_detail');
    } else if (action === 'navigate_cases') {
      onNavigate('case_management');
    } else if (action === 'navigate_monitoring') {
      onNavigate('transaction_monitoring');
    } else if (action === 'settings') {
      onNavigate('system_settings');
    }
  };
