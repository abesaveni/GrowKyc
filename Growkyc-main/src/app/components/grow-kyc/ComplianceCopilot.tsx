...
export function ComplianceCopilot({ isOpen, onClose, context }: ComplianceCopilotProps) {
  ...
  const generateAIResponse = (query: string): Message => {
    ...
    if (context?.clientId || context?.caseId || context?.page) {
      const ids = [];
      if (context.clientId) ids.push(`Client ${context.clientId}`);
      if (context.caseId) ids.push(`Case ${context.caseId}`);
      if (context.page) ids.push(`the ${context.page} page`);
      contextAcknowledgment = `I notice you are currently viewing ${ids.join(', ')}.\n\n`;
    }
