...
export function Pipeline({ onNavigate, onBack }: PipelineProps) {
  ...
  const stats = [
    { label: 'Total Pipeline Value', value: '$5.32M', icon: DollarSign, color: 'text-blue-400', bg: 'bg-blue-500/10' },
    { label: 'Active Deals', value: '24', icon: TrendingUp, color: 'text-green-400', bg: 'bg-green-500/10' },
    { label: 'Avg. Days in Pipeline', value: '12', icon: Clock, color: 'text-orange-400', bg: 'bg-orange-500/10' },
    { label: 'Conversion Rate', value: '68%', icon: CheckCircle, color: 'text-purple-400', bg: 'bg-purple-500/10' }
  ];
  ...
  const getStageBadge = (stage: string) => {
    const stages = {
      ...
      assessment: 'bg-orange-500/15 text-orange-300',
      approved: 'bg-green-500/15 text-green-300',
      settlement: 'bg-teal-500/15 text-teal-300'
    };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-300">Track and manage deals through the lending pipeline</p>
            </div>
          </div>
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          {stats.map((stat, index) => (
            <div key={index} className="bg-white border border-white/10 rounded p-4">
              <div className="flex items-center justify-between">
                <div>
        ...
        {/* Pipeline Stages */}
        <div className="bg-white border border-white/10 rounded p-4 mb-6">
          <h2 className="text-sm font-semibold text-slate-100 mb-3">Pipeline Stages</h2>
            ...
            {pipelineStages.map((stage) => (
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs font-medium text-slate-100">{stage.name}</span>
                </div>
        ...
        <div className="space-y-4">
          {filteredDeals.map((deal) => (
            <div key={deal.id} className="bg-white border border-white/10 rounded p-6 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="text-lg font-bold text-slate-100">{deal.client}</h3>
                  ...
                  <p className="text-sm text-slate-300">Application ID: {deal.id}</p>
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
                <div>
                  <p className="text-xs text-slate-300 mb-1">Broker</p>
                  <div className="flex items-center gap-2">
                    <Users className="w-4 h-4 text-slate-400" />
                    <p className="text-sm font-medium text-slate-100">{deal.broker}</p>
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-medium text-slate-100">{deal.loanType}</p>
                  </div>
                </div>
                <div>
                  <p className="text-xs text-slate-300 mb-1">Amount</p>
                  <div className="flex items-center gap-2">
                    <DollarSign className="w-4 h-4 text-slate-400" />
                    <p className="text-sm font-bold text-slate-100">{deal.amount}</p>
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-medium text-slate-100">{deal.daysInStage} days</p>
                  </div>
                </div>
                <div>
                  <p className="text-xs text-slate-300 mb-1">Last Update</p>
                  <div className="flex items-center gap-2">
                    <Calendar className="w-4 h-4 text-slate-400" />
                    <p className="text-sm font-medium text-slate-100">{deal.lastUpdate}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-4">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <p className="text-xs font-medium text-slate-100">{deal.progress}%</p>
                </div>
        ...
        {filteredDeals.length === 0 && (
          <div className="bg-white border border-white/10 rounded p-12 text-center">
            <FileText className="w-12 h-12 text-gray-400 mx-auto mb-3" />
            <h3 className="text-lg font-semibold text-slate-100 mb-2">No deals found</h3>
