...
export function DealRooms({ onNavigate, role, onBack }: DealRoomsProps) {
  ...
  const statusConfig = {
    active: { label: 'Active', color: 'bg-green-500/15 text-green-300 border-green-300', icon: CheckCircle },
    closed: { label: 'Closed', color: 'bg-white/5 text-slate-300 border-white/10', icon: Lock },
    pending: { label: 'Pending', color: 'bg-yellow-500/15 text-yellow-300 border-yellow-300', icon: Clock }
  };

  const categoryConfig = {
    financials: { label: 'Financial', color: 'bg-blue-500/15 text-blue-300', icon: FileText },
    legal: { label: 'Legal', color: 'bg-purple-500/15 text-purple-300', icon: FileText },
    operational: { label: 'Operational', color: 'bg-green-500/15 text-green-300', icon: FileText },
    marketing: { label: 'Marketing', color: 'bg-orange-500/15 text-orange-300', icon: FileText },
    other: { label: 'Other', color: 'bg-white/5 text-slate-300', icon: FileText }
  };
  ...
  const getFileIcon = (type: string) => {
    switch (type.toLowerCase()) {
      ...
      case 'archive':
        return <Archive className="w-8 h-8 text-slate-300" />;
      default:
      ...
      <div className="p-8 space-y-6">
        <div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-slate-100">Deal Rooms</h1>
              <p className="text-slate-300 mt-1">Secure virtual data rooms for deal documentation and investor Q&A</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-300">Total Deal Rooms</p>
                  <p className="text-3xl font-bold text-slate-100 mt-1">{dealRooms.length}</p>
                </div>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-300">Active Rooms</p>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-300">Open Questions</p>
                ...
                <CardContent className="p-6">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-slate-300 mb-4">{room.dealName}</p>
                      ...
                      <div className="grid grid-cols-4 gap-6">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium text-slate-300 mt-1">{room.lastActivity}</p>
                        </div>
                      </div>

                      <div className="mt-4 flex items-center gap-4 text-sm text-slate-300">
                        <span>Fund Manager: <span className="font-medium text-slate-100">{room.fundManager}</span></span>
                      </div>
                    </div>
                  </div>
                </CardContent>
    ...
    <div className="p-8 space-y-6">
      <div>
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              <h1 className="text-3xl font-bold text-slate-100">{currentRoom.name}</h1>
              {currentRoom.confidential && (
                <Lock className="w-6 h-6 text-red-400" />
              )}
            </div>
            <p className="text-slate-300">{currentRoom.dealName}</p>
          </div>
          {(role === 'fund-manager' || role === 'fund-accountant') && (
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-100">{currentRoom.participants}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-100">{currentRoom.documents}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-100">{currentRoom.questions}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-slate-300">Last Activity</p>
                <p className="text-sm font-medium text-slate-100">{currentRoom.lastActivity}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          {activeTab === 'documents' && (
            <div className="space-y-4">
              {documents.map((doc) => {
                ...
                return (
                  <div key={doc.id} className="p-4 bg-white/5 rounded-lg border border-white/10 hover:shadow-md transition-all">
                ...
                <CardContent className="p-4">
                  <div className="flex gap-3">
                    <textarea
                      ...
                      rows={3}
                      className="flex-1 px-3 py-2 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white"
                    />
                    <Button className="bg-indigo-600 hover:bg-indigo-700">
                      ...
                      <div className="flex items-start gap-4">
                        <div className="flex-1">
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex-1">
                              <h3 className="font-semibold text-slate-100 mb-2">{q.question}</h3>
                            ...
                            <span
                              className={`px-3 py-1 rounded-full text-xs font-semibold ${
                                q.status === 'answered'
                                  ...
                                  : q.status === 'pending'
                                  ? 'bg-orange-500/15 text-orange-300 border border-orange-300'
                            ...
                            <div className="mt-4 p-4 bg-green-500/10 rounded-lg border border-green-500/30">
                              <div className="flex items-start gap-2 mb-2">
                                ...
                                <div className="flex-1">
                                  ...
                                  <p className="text-sm text-slate-300 mt-2">{q.answer}</p>
