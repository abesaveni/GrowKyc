...
const REQUIRED_ENV_VARS: EnvVarSpec[] = [
  ...
  { key: 'PEXA_CLIENT_ID',                reasonCode: 'missing_env_var',     description: 'PEXA OAuth client ID' },
  { key: 'PEXA_CLIENT_SECRET',            reasonCode: 'missing_env_var',     description: 'PEXA OAuth client secret' },
  { key: 'PEXA_BASE_URL',                 reasonCode: 'missing_env_var',     description: 'PEXA API base URL' },
  { key: 'PEXA_WEBHOOK_SECRET',           reasonCode: 'missing_env_var',     description: 'PEXA webhook HMAC secret' },
];
...
function checkOAuthAndApiConfig(
  env: EnvMap,
  failures: ReadinessCheckFailure[]
): void {
  const pexaBaseUrl = env['PEXA_BASE_URL'];

  if (pexaBaseUrl) {
    try {
      const url = new URL(pexaBaseUrl);
      if (url.protocol !== 'https:') {
        addFailure(
          failures,
          'invalid_env_var_value',
          'PEXA_BASE_URL',
          'PEXA_BASE_URL must use https in production.'
        );
      }
    } catch {
      addFailure(
        ...
        'invalid_env_var_value',
        'PEXA_BASE_URL',
        `PEXA_BASE_URL value "${pexaBaseUrl}" is not a valid URL.`
      );
    }
  }

  if (env['PEXA_CLIENT_ID'] && !env['PEXA_CLIENT_SECRET']) {
    addFailure(
      failures,
      'invalid_env_var_value',
      'PEXA_CLIENT_SECRET',
      'PEXA_CLIENT_SECRET is required when PEXA_CLIENT_ID is configured.'
    );
  }
}
...
export const validateProductionReadinessFromEnv = (
  ...
  checkOAuthAndApiConfig(env, failures);
